...
class LOCM2(OCM):
    ...
    TransitionSet = Dict[FSM, Dict[TypedObject, List[SingletonEvent]]]
    ...
    def extract_model(self, trace_list, types=None):
        ...
        transition_sets_per_sort_list = self.split_transitions(TM_list, debug=self.debug)
        ...
        for sort, TM in enumerate(TM_list):
            ...
            for i in range(TM.shape[0]):
                for j in range(TM.shape[1]):
                    if TM.iloc[i, j] == -1:
                        holes.add(frozenset({TM.index[i], TM.columns[j]}))
    ...
    def get_consecutive_transitions(self,TM_list_with_holes):
        consecutive_transitions_per_sort_list = []
