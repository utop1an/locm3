...
class POLOCM2(LOCM2):
    ...
    def get_TS_OS(self, obj_traces_list, transition_sets_per_sort_list, TM_list, sorts: LOCM2.SortDict, debug=False):
    ...
    def get_state_bindings(self,TS_list, ap_state_pointers, OS, sorts, debug=False):
        hs = self._step3(TS_list, ap_state_pointers, OS, sorts, debug)
    ...
    def _step4(self,HS: Hypothesis, debug: bool = False) -> Bindings:
        ...
        for sort, hs_sort in HS.items():
            for state, hs_sort_state in hs_sort.items():
                ...
                bindings[sort][state] = [
                    LOCM2.Binding(h, LOCM2._pointer_to_set(state_params, v)[0])
    ...
    def get_model(self, OS, ap_state_pointers, sorts, bindings, sort_to_type_dict=None, statics=[],  debug=False):
        ...
        types = LOCM2._sorts_to_types(sorts, None)
