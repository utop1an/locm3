...
class OCM(ABC):
    ...
    def trace_to_obj_trace2(self, trace_list, sorts, debug=False):
        """
        Convert a list of traces to a list of object traces.
        Each object trace is a dictionary mapping each object to a list of events.
    ...
    def obj_tuple_to_sort_tuple(self, obj_tuple: tuple, sorts: SortDict) -> tuple:
        """Convert a tuple of objects to a tuple of sorts.

        :param obj_tuple: Tuple of objects to convert.
        :param sorts: Dictionary mapping object names to sorts.

        :return: Tuple of sorts corresponding to the objects in obj_tuple.
        """
        return tuple(sorts[obj.name] if obj==self.ZEROOBJ else 0 for obj in obj_tuple)
